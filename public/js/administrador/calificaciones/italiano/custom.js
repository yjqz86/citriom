$(document).ready(function(){
    ...
    $.ajax({
      ...
      success:  function (data) {
        for(var i = 0; i < data.inscritosuba.length; i++){
          $("table tbody").append("<tr class='text-center'><td hidden>"+data.inscritosuba[i].id+"</td><td>"+(i+1)+"</td><td>"+data.inscritosuba[i].cedula+"</td><td>"+data.inscritosuba[i].estudiante.NOM_APE+"</td><td><input type='text' class='form-control primera-nota soloNumerosPunto' maxlength = '2' value='"+(data.inscritosuba[i].notaparcial != null ? data.inscritosuba[i].notaparcial.corte1 : '')+"'/></td><td><input type='text' class='form-control segunda-nota soloNumerosPunto' maxlength = '2' value='"+(data.inscritosuba[i].notaparcial != null ? data.inscritosuba[i].notaparcial.corte2 : '')+"' /></td><td><span class='notaEscalaCien'>"+(data.inscritosuba[i].notaparcial != null ? data.inscritosuba[i].notaparcial.corte1+data.inscritosuba[i].notaparcial.corte2 : '')+"</span></td><td><span class='nota-final'>"+(data.inscritosuba[i].notaparcial != null ? ( Math.round(((data.inscritosuba[i].notaparcial.corte1+data.inscritosuba[i].notaparcial.corte2)*20)/100) == 0 ? '1' : Math.round(((data.inscritosuba[i].notaparcial.corte1+data.inscritosuba[i].notaparcial.corte2)*20)/100) ) : '')+"</span></td><td><button type='button' class='btn btn-primary guardar' disabled><span class='fui-check'></span></button></td><td><span class='response' hidden></span></td></tr>");
        }
        validar();
        mostrar();
        modal.style.display = "none";
  ...
  $(document).on('keyup','input[type=text].primera-nota,input[type=text].segunda-nota',function(){
    ...
    if(actual.hasClass('primera-nota') && nota1 > 40){
      actual.addClass("has-error");
    }else if(actual.hasClass('segunda-nota') && nota2 > 60){
      actual.addClass("has-error");
    }else{
    ...
    var suma = (parseFloat(nota1)+parseFloat(nota2));
