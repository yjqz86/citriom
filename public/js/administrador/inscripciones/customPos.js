$(document).ready(function(){
  ...
  $("#cedula").change(function(){
    ...
    ocultar();
    $("select:not('.not')").each(function(){
      $(this).empty();
    });
    $("#carrera").html("<option value='' hidden>Seleccione...</option>");
    $("#carrera").select2("val", "");
    $('#aula').val('');
    $('#horario').val('');
    $.ajax({
      ...
      url:   'inscripcion-estudiante/obtener-estudiante-postgrado',
      type:  'post',

      beforeSend: function(){
        $('#nombres').val('Buscando...');
        $('#carrera').val('Buscando...');
        $('#nucleo').val('Buscando...');
      ...
      success:  function (data) {
       if(data.error == 0){
          ...
          for(var i = 0; i<data.codigos.length; i++){
            $('#carrera').append('<option value="'+data.codigos[i]+'">'+data.nombreCarrera[i]+'</option>');
          }

        } else if(data.error==1){
          $("#mensaje-error").text("Este estudiante ya se encuentra cursando un nivel de idiomas");
          $("#error").show();
        } else if(data.error==2){
          $("#mensaje-error").text("Este estudiante ya ha aprobado todos los niveles de idiomas");
          $("#error").show();
        }else if(data.error==3){
          $("#mensaje-error").text("Este seccion esta llena");
          $("#error").show();
        }else if(data.error==4){
          $("#mensaje-error").text("Ha ocurrido un error inscribiendo al estudiante");
          $("#error").show();
      }
  ...
  $("#carrera").change(function(){
    ...
    $('#aula').val('');
    $('#horario').val('');
    $("#turno").html('<option hidden value="" selected> Selecione...</option><option disabled>No hay turnos disponibles</option>');
    $("#seccion").html('<option hidden value="" selected></option><option disabled>No hay secciones disponibles</option>');
    $("#turno").select2("val", "");
    $("#seccion").select2("val","");
    ...
    $.ajax({
      ...
      url:   'inscripcion-estudiante/obtener-curso',
      type:  'post',
      success:  function (data) {
      	console.log(data);
        if(data  != ''){
        ...
        for(var i = 0; i<data.length; i++){
          $('#curso').append('<option value="'+data[i].id+'">'+data[i].nivel+'</option>');
          if(data[i].tipoEstudiante==1){
            $('#curso').select2('val', '');
            tipoEstudiante = 1;
          }else{
            $('#curso').select2('val', data[i].id, true);
            tipoEstudiante = 2;
          }
        }
        ocultarPagos();
  ...
  $("#curso").change(function(){
    $.ajax({
      ...
      url:   'inscripcion-estudiante/obtener-turnos-postgrado',
      type:  'post',
      success:  function (data) {
        $('#turno').html(data);
        if (tipoEstudiante == 1) {
          ...
          $('#turno').select2('val', 'Semi-Presencial', true);
  ...
  $("#turno").change(function(){
    ...
    $.ajax({
      ...
      url:   'inscripcion-estudiante/obtener-secciones-postgrado',
      type:  'post',
      success:  function (data) {
        $("#seccion").select2("val","");
        $('#seccion').html(data);
  ...
  $("#seccion").change(function(){
    $.ajax({
      ...
      url:   'inscripcion-estudiante/obtener-final-postgrado',
      type:  'post',
      success:  function (data) {
        console.log(data);
        $("#aula").val(data.aula);
