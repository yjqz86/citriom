$(document).ready(function(){
  ...
  $("#curso").change(function(){
    ...
    if($("#lapso").val() != '' && $("#nucleo").val()){
      $.ajax({
        ...
        url:   'obtener-secciones-postgrado',
        type:  'post',
  ...
  $("#seccion").change(function(){
    ...
    $.ajax({
      ...
      url: 'obtener-estudiantes-postgrado',
      type: 'post',
      success:  function (data) {
        for(var i = 0; i < data.inscritosposgrado.length; i++){
          $("table tbody").append("<tr class='text-center'><td hidden>"+data.inscritosposgrado[i].id+"</td><td>"+(i+1)+"</td><td>"+data.inscritosposgrado[i].cedula+"</td><td>"+data.inscritosposgrado[i].estudiante.nombres+
          " "+data.inscritosposgrado[i].estudiante.apellidos+"</td><td><input type='text' class='form-control primera-nota soloNumerosPunto' value='"+(data.inscritosposgrado[i].notaparcial != null ? data.inscritosposgrado[i].notaparcial.corte1 : '')+"'/></td><td><input type='text' class='form-control segunda-nota soloNumerosPunto'  value='"+(data.inscritosposgrado[i].notaparcial != null ? data.inscritosposgrado[i].notaparcial.corte2 : '')+"' /></td><td><span class='notaEscalaCien'>"+(data.inscritosposgrado[i].notaparcial != null ? parseInt(data.inscritosposgrado[i].notaparcial.corte1+data.inscritosposgrado[i].notaparcial.corte2) : '')+"</span></td><td><span class='nota-final'>"+(data.inscritosposgrado[i].notaparcial != null ? ( Math.round(((data.inscritosposgrado[i].notaparcial.corte1+data.inscritosposgrado[i].notaparcial.corte2)*20)/100) == 0 ? '1' : Math.round(((data.inscritosposgrado[i].notaparcial.corte1+data.inscritosposgrado[i].notaparcial.corte2)*20)/100) ) : '')+"</span></td><td><button type='button' class='btn btn-primary guardar' disabled><span class='fui-check'></span></button></td><td><span class='response' hidden></span></td></tr>");
        }
        validar();
        mostrar();
        modal.style.display = "none";
  ...
  $("#aceptarValidar").click(function(){
    ...
    $.ajax({
      ...
      url:   'validar-notas-postgrado',
      type:  'post',
      success: function (data){
        window.location = 'postgrado';
      }
    });
  });
